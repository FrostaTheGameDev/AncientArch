...
public class CrackedEndPortalFrameBlock extends EndPortalFrameBlock{
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (itemStack.isOf(ArchItems.HAMMER_AND_CHISEL)) {
            if (!world.isClient) {
                Direction direction = hit.getSide();
                Direction direction2 = direction.getAxis() == Direction.Axis.Y ? player.getHorizontalFacing().getOpposite() : direction;
                world.playSound(null, pos, SoundEvents.BLOCK_GLASS_BREAK, SoundCategory.BLOCKS, 1.0F, 1.0F);
                world.setBlockState(
                        pos, ArchBlocks.CRACKED_END_PORTAL_FRAME_USED.getDefaultState().with(CrackedEndPortalFrameUsedBlock.FACING, direction2), Block.NOTIFY_ALL | Block.REDRAW_ON_MAIN_THREAD
                );
                ItemEntity itemEntity = new ItemEntity(
                        ...
                        new ItemStack(ArchItems.REMNANT_SHARD, 1)
                );
                ...
                player.incrementStat(Stats.USED.getOrCreateStat(ArchItems.HAMMER_AND_CHISEL));
            }

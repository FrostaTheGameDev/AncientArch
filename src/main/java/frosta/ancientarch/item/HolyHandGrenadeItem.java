...
public class HolyHandGrenadeItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            HolyHandGrenadeEntity holyhandgrenadeEntity = new HolyHandGrenadeEntity(user, world);
            holyhandgrenadeEntity.setItem(itemStack);
            holyhandgrenadeEntity.setVelocity(user, user.getPitch(), user.getYaw(), 0.0f, 1.5f, 1.0f);
            world.spawnEntity(holyhandgrenadeEntity);
        }
